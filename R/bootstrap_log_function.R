#' The bootstrap_log function
#'


bootstrap_log_function <- function(variables, number, model_data, ndraws, l = "lambda.1se") {
  ...
  extract_names <- glm(as.formula(paste("dmNewDig ~", paste0(gsub("0.*","", vals[,1]), "0", collapse = " + "))), data = dataCon, family = "binomial") %>%
    tidy()
  ...
  for (i in 1:ndraws) {
    ...
    bootstrap_model <- glm(as.formula(paste("dmNewDig ~", paste0(gsub("0.*","", names(as.matrix(coef(m1e))[as.matrix(coef(m1e))[,1] != 0,])), "0", collapse = " + "))), family = "binomial", data = dataCon)
